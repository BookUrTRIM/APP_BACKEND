...
class ServiceService:
    @staticmethod
    def create(user_account_id: int, dto: ServiceCreateDTO) -> ServiceResponseDTO:
        provider = ProviderRepository.get_by_user_account_id(user_account_id)
        if not provider:
            raise ProviderNotFound()

        service = ServiceRepository.create(dto, provider.id)
        logger.info("Prestation créée : id=%d provider_id=%d", service.id, service.provider_id)
        return ServiceMapper.model_to_dto(service)

    @staticmethod
    def update(service_id: int, user_account_id: int, dto: ServiceUpdateDTO) -> ServiceResponseDTO:
        ...
        updated = ServiceRepository.update(service_id, dto)
        return ServiceMapper.model_to_dto(updated)

    @staticmethod
    def delete(service_id: int, user_account_id: int) -> None:
        service = ServiceRepository.get_by_id(service_id)
        if not service:
            raise ServiceNotFound()
    ...
    @staticmethod
    def get(service_id: int) -> ServiceResponseDTO:
        service = ServiceRepository.get_by_id(service_id)
        if not service:
            raise ServiceNotFound()
        return ServiceMapper.model_to_dto(service)
    ...
    @staticmethod
    def list_by_provider(provider_id: int, page: int = 1, limit: int = 20) -> Tuple[List[ServiceResponseDTO], int]:
        services, total = ServiceRepository.list_by_provider(provider_id, page=page, limit=limit)

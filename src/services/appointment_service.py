...
class AppointmentService:
    @staticmethod
    def book(db: Session, user_account_id: int, dto: AppointmentCreateDTO) -> AppointmentResponseDTO:
        client = ClientRepository.get_by_user_account_id(db, user_account_id)
        if not client:
            raise ClientNotFound()

        if not ProviderRepository.get_by_id(db, dto.provider_id):
            raise ProviderNotFound()

        if AppointmentRepository.count_pending_by_client(db, client.id) >= 2:
            raise TooManyPendingAppointments()

        if AppointmentRepository.has_conflict(db, dto.provider_id, dto.start_at, dto.end_at):
            raise AppointmentSlotUnavailable()

        appointment = AppointmentRepository.create(db, dto, client.id)
        db.commit()
    ...
    @staticmethod
    def update(db: Session, appointment_id: int, user_account_id: int, current_role: UserRole, dto: AppointmentUpdateDTO) -> AppointmentResponseDTO:
        ...
        updated = AppointmentRepository.update(db, appointment_id, dto)
        db.commit()
        return AppointmentMapper.model_to_dto(updated)

    @staticmethod
    def cancel_by_client(db: Session, appointment_id: int, user_account_id: int) -> AppointmentResponseDTO:
        appointment = AppointmentRepository.get_by_id(db, appointment_id)
        if not appointment:
            raise AppointmentNotFound()
        ...
        if appointment.status == AppointmentStatus.CONFIRMED:
            AvailabilityRepository.delete_booked(db, appointment.provider_id, appointment.start_at)
        db.commit()
        logger.info("RDV annulé par client : id=%d", appointment_id)
        return AppointmentMapper.model_to_dto(updated)

    @staticmethod
    def cancel_by_provider(db: Session, appointment_id: int, user_account_id: int) -> AppointmentResponseDTO:
        appointment = AppointmentRepository.get_by_id(db, appointment_id)
        if not appointment:
            raise AppointmentNotFound()
        if appointment.status == AppointmentStatus.CANCELLED:
        ...
        if payment and payment.stripe_charge_id:
            try:
                ...
                PaymentRepository.update_status_by_id(db, payment.id, PaymentStatus.REFUNDED)
                db.commit()
                ...
                AppointmentRepository.update_status(db, appointment_id, appointment.status)
                db.commit()
                raise PaymentFailed()
        else:
            db.commit()
    ...
    @staticmethod
    def complete(db: Session, appointment_id: int, user_account_id: int) -> AppointmentResponseDTO:
        ...
        updated = AppointmentRepository.update_status(db, appointment_id, AppointmentStatus.COMPLETED)
        db.commit()
    ...
    @staticmethod
    def cancel(db: Session, appointment_id: int, user_account_id: int, current_role: UserRole) -> AppointmentResponseDTO:
        appointment = AppointmentRepository.get_by_id(db, appointment_id)
        if not appointment:
            raise AppointmentNotFound()
        if appointment.status == AppointmentStatus.CANCELLED:
    ...
    @staticmethod
    def get(db: Session, appointment_id: int, user_account_id: int, current_role: UserRole) -> AppointmentResponseDTO:
        appointment = AppointmentRepository.get_by_id(db, appointment_id)
        if not appointment:
            raise AppointmentNotFound()
        AppointmentService.assert_access(db, appointment, user_account_id, current_role)
    ...
    @staticmethod
    def list_by_client(db: Session, user_account_id: int, status: Optional[AppointmentStatus] = None, page: int = 1, limit: int = 20) -> Tuple[List[AppointmentResponseDTO], int]:
        client = ClientRepository.get_by_user_account_id(db, user_account_id)
        if not client:
            raise ClientNotFound()
        appointments, total = AppointmentRepository.list_by_client(db, client.id, status=status, page=page, limit=limit)
        return [AppointmentMapper.model_to_dto(a) for a in appointments], total

    @staticmethod
    def list_by_provider(db: Session, user_account_id: int, status: Optional[AppointmentStatus] = None, page: int = 1, limit: int = 20) -> Tuple[List[AppointmentResponseDTO], int]:
        provider = ProviderRepository.get_by_user_account_id(db, user_account_id)
        if not provider:
            raise ProviderNotFound()
        appointments, total = AppointmentRepository.list_by_provider(db, provider.id, status=status, page=page, limit=limit)
        return [AppointmentMapper.model_to_dto(a) for a in appointments], total
    ...
    def _assert_valid_transition(current: AppointmentStatus, target: AppointmentStatus) -> None:
        if target not in _VALID_TRANSITIONS.get(current, set()):
            raise InvalidStatusTransition(
                detail=f"Transition '{current}' → '{target}' non autorisée."
            )

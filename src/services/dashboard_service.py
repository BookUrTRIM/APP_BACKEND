...
class DashboardService:
    @staticmethod
    def get_metrics(db: Session, user_account_id: int, period: str) -> DashboardModel:
        ...
        appts = DashboardRepository.get_all_appointments(db, provider.id)
        services_sold = DashboardRepository.get_all_appointment_services(db, provider.id)
        ...
        filtered_appts = [
            a for a in appts
            if a.start_at and start_date <= a.start_at.replace(tzinfo=None) <= end_date
        ]
        ...
        top_service_name = DashboardRepository.get_top_service_name(db, filtered_appt_ids)
        avg_rating = DashboardRepository.get_average_rating(db, provider.id)
        ...
        for i in range(5, -1, -1):
            ...
            month_appts = [
                a for a in appts
                if a.start_at
                and a.start_at.replace(tzinfo=None).month == target_month
                and a.start_at.replace(tzinfo=None).year == target_year
            ]
